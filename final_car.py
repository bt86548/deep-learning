...
train_root=r'C:\Users\Big data\Desktop\HOT\img_distinct\classify\test_classify'
# vaildation_root=r'C:\Users\Big data\Desktop\HOT\img_distinct\classify\test_classify'
# test_root=r'C:\Users\Big data\Desktop\HOT\img_distinct\classify\test_classify'
...
def check_print():
    ...
    model.compile(optimizer=opt,loss='categorical_crossentropy',metrics=['acc',metric.top_k_categorical_accuracy])
    # model.compile(optimizer='adam',loss='categorical_crossentropy',metrics=['acc',metric.top_k_categorical_accuracy])
    print('Model Compiled')
    return model

if __name__=='__main__':
    if os.path.exists(r'C:\Users\Big data\Desktop\HOT\img_distinct\inception_3.h5'):
        model=load_model(r'C:\Users\Big data\Desktop\HOT\img_distinct\inception_3.h5')
    else:
        model=check_print()
    es = EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=5)
    H = model.fit_generator(train_generator,validation_data=vaild_generator,epochs=EPOCH,steps_per_epoch=train_generator.n/batch_size
                        ,validation_steps=vaild_generator.n/batch_size,shuffle=True,callbacks=[es])
    ...
    plt.ylabel('loss')
    plt.legend()
    plt.savefig('loss_car.png')

    plt.show()



    model.save(r'C:\Users\Big data\Desktop\HOT\img_distinct\inception_3.h5')
